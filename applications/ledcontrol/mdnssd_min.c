...
int print_answers(FoundAnswerList* alist, char *ip, int *port) {
  ...
  for(i=0; i < alist->length; i++) {
    ...
    sprintf(ip, "%s", inet_ntoa(answer->addr));
    *port = answer->port;
...
void complete_answers(int sock, char* query_arg, FoundAnswerList* alist) {
  ...
  for(i=0; i < alist->length; i++) {
    ...
    if(a->rr->type != DNS_RR_TYPE_PTR && a->rr->type != DNS_RR_TYPE_SRV) {
      continue;
    }
...
int getService(char * srv_name, char *ip, int *port)
{
  ...
  ret = print_answers(&alist, ip, port);

  clear_answer_list(&alist);

  debug("---getService--end-\n");
